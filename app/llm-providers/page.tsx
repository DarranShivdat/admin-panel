...
export default function LLMProvidersPage() {
  ...
  useEffect(() => {
    supabase.from("llm_providers").select("*").order("id").then(({ data }) => setData(data ?? []));
  }, []);
  ...
  const handleUpdate = async (id: number) => {
    await supabase.from("llm_providers").update({ name: editName }).eq("id", id);
    setData(data.map((d) => d.id === id ? { ...d, name: editName } : d));
    setEditingId(null);
  };

  const handleAdd = async () => {
    const { data: inserted } = await supabase.from("llm_providers").insert({ name: newName }).select().single();
    if (inserted) { setData([...data, inserted]); setAdding(false); setNewName(""); }
  };
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        {adding && (
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 mb-4 flex gap-3">
            <input className="flex-1 bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm text-white" placeholder="Provider name" value={newName} onChange={(e) => setNewName(e.target.value)} />
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="border-b border-gray-800 text-gray-400">
              <tr>{["ID", "Name", "Actions"].map((h) => <th key={h} className="text-left px-4 py-3 font-medium">{h}</th>)}</tr>
            </thead>
            <tbody>
              {data.map((row) => (
                  ...
                  {editingId === row.id ? (
                    <td colSpan={2} className="px-4 py-3">
                      <div className="flex gap-2">
                        <input className="bg-gray-800 border border-gray-700 rounded px-2 py-1 text-xs text-white flex-1" value={editName} onChange={(e) => setEditName(e.target.value)} />
                    ...
                    <>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button onClick={() => { setEditingId(row.id); setEditName(row.name); }} className="text-xs bg-gray-700 text-gray-200 px-2 py-1 rounded">Edit</button>
